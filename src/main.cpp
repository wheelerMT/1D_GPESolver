...
double normalise(std::complex<double> *t_wfn, int t_arrayLength, double t_dx) {
  ...
  for (int i = 0; i < t_arrayLength; ++i) {
    sum += (abs(t_wfn[i]) * abs(t_wfn[i])) * t_dx;
  }
...
int main() {
  ...
  p_forward = fftw_plan_dft_1d(Nx, reinterpret_cast<fftw_complex *>(psi),
                               reinterpret_cast<fftw_complex *>(psi_k),
                               FFTW_FORWARD, FFTW_ESTIMATE);
  p_back = fftw_plan_dft_1d(Nx, reinterpret_cast<fftw_complex *>(psi_k),
  ...
  for (int i = 0; i < Nt; ++i) {
    ...
    for (int j = 0; j < Nx; ++j) {
      ...
      psi[j] =
          psi[j] * exp(-0.5 * dt * (V[j] + c0 * abs(psi[j]) * abs(psi[j])));
    }
